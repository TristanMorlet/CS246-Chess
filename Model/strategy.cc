...
Move Level4::chooseMove(const Board& b, Colour side) {
    ...
    if (v.empty()) return {};
    double moveValue = -10.0; // Value of a move
    double oldThem = 0; // Value of target piece
    double oldMe = 0; // Value of my piece
    std::vector<Move> satisfied;
    Move theMove{}; // The Move
    for (const auto& m : v) {
        const Piece* target = b.getPieceAt(m.to); // Get target square
        const Piece* piece = b.getPieceAt(m.from); // Get our piece

    // MOVE LOGIC

        // First and foremost, if we can checkmate, we do it.
        if (givesMate(b, m, side)) return m;
        // Then we check if we can check
        if (moveValue < S_CHECK && givesCheck(b, m, side)) {
            theMove = m;
            moveValue = S_CHECK;
            continue;
        }
    
        // Next, if we can promote a pawn, we do that.
        if ((moveValue < S_PROMO) && !target && ((piece->getCharRepresentation() == 'P' && m.to.row == 7) || 
        (piece->getCharRepresentation() == 'p' && m.to.row == 0)) && safeAfter(b, m, side)) {
            theMove = m;
            moveValue = S_PROMO;
            continue;
        }

        // Next we get the best capturing move, prioritizing material diff
        if (target && target->getColour() != side) {
            double me = piece->getMaterialValue();
            double them = target->getMaterialValue();
            double currval = safeAfter(b, m, side) ? them : (them - me);
            if (currval >= 0 && (currval > moveValue || (currval == moveValue && them >= oldThem))) {
            theMove = m;
            moveValue = std::min(S_SAFE_CAP + currval, S_PROMO - 0.5); 
            oldThem = them;
            continue;
            }
            else if (currval >= 2 && moveValue < S_UNSAFE_CAP) {
                theMove = m;
                moveValue = S_UNSAFE_CAP;
                continue;
                }
                
        }

        // Almost finally, if there are no checks and no good capturing moves, save a hanging piece
        if (!target && b.isDanger(m.from, side) && safeAfter(b, m, side)) {
            double currval = piece->getMaterialValue();
            if (currval > moveValue || (currval == moveValue && currval >= oldMe)) {
                theMove   = m;
                moveValue = S_SAVE;
                oldMe     = currval;
                }
        ...
        // I there are no pieces in danger, no checks, and no capturing moves, we castle if we can.
        if (moveValue < S_CASTLE && !target &&
    std::tolower(piece->getCharRepresentation()) == 'k' &&
...
// THIS SHOULD BE RANDOM here.
        // If there are no checks, no pieces in danger, no capturing moves, and we can't castle, we just play a move.
        if ((moveValue < S_QUIET) && !target && safeAfter(b, m, side)) {
               satisfied.push_back(m);          // quiet pool
                continue;
}
    }

if (moveValue < S_QUIET && !satisfied.empty()) {
    std::uniform_int_distribution<size_t> d(0, satisfied.size()-1);
    return satisfied[d(rng)];
}
if (!b.isMoveValid(theMove)) {                     // last safety net
    std::uniform_int_distribution<size_t> d(0, v.size()-1);
    return v[d(rng)];
}
return theMove;
